...
- Normalize into two small tables:
    silver/dim_npi/dim_npi.parquet              (1 row per NPI)
    silver/dim_npi_address/dim_npi_address.parquet  (N rows per NPI: LOCATION/MAILING)
...
CLI:
    python utils_nppes.py --silver-dir core/data/silver --add 1295795499 1639195834
    python utils_nppes.py --silver-dir core/data/silver --file npis.txt
"""
...
def ensure_dirs(base: Path) -> Tuple[Path, Path]:
    dim_dir = base / "dim_npi"
    addr_dir = base / "dim_npi_address"
    dim_dir.mkdir(parents=True, exist_ok=True)
    addr_dir.mkdir(parents=True, exist_ok=True)
    return dim_dir, addr_dir

def dim_paths(silver_dir: Path, dim_npi_filename: str = "dim_npi.parquet") -> Tuple[Path, Path]:
    ...
    """
    dim_dir, addr_dir = ensure_dirs(silver_dir)
...
# ------------- PUBLIC API ------------------
def add_npi_to_dims(npi: str, silver_dir: str | Path, dim_npi_filename: str = "dim_npi.parquet") -> bool:
    """
    High-level helper:
    ...
    dim_path, addr_path = dim_paths(Path(silver_dir), dim_npi_filename)
...
def add_many_npis(npis: List[str], silver_dir: str | Path, sleep: float = 0.0, dim_npi_filename: str = "dim_npi.parquet") -> Tuple[int,int]:
    """
...
def get_npi_info(npi: str, silver_dir: str | Path = "core/data/silver", dim_npi_filename: str = "dim_npi.parquet") -> Optional[Dict[str, Any]]:
    """
    ...
    """
    dim_path, _ = dim_paths(Path(silver_dir), dim_npi_filename)
...
def list_available_npis(silver_dir: str | Path = "core/data/silver", dim_npi_filename: str = "dim_npi.parquet", limit: int = 100) -> pl.DataFrame:
    """
...
def batch_update_npis_from_fact(silver_dir: str | Path = "core/data/silver", 
                               fact_path: str = "core/data/gold/fact_rate.parquet",
    ...
    """
    # Read fact table and extract unique NPIs from xref_group_npi
    xref_path = Path(silver_dir) / "xref_group_npi" / "xref_group_npi.parquet"
    
    if not xref_path.exists():
        print(f"xref_group_npi not found at {xref_path}")
        return 0, 0
    
    xref_df = pl.read_parquet(str(xref_path))
    unique_npis = xref_df.select("npi").unique().to_series().to_list()
    
    print(f"Found {len(unique_npis)} unique NPIs in xref_group_npi")
    
    # Update dim_npi with these NPIs
    return add_many_npis(unique_npis, silver_dir, sleep=sleep, dim_npi_filename=dim_npi_filename)
...
def add_npi_placeholders(npis: List[str], silver_dir: str | Path, dim_npi_filename: str = "dim_npi.parquet") -> int:
    """
...
def backfill_missing_npis(silver_dir: str | Path, dim_npi_filename: str = "dim_npi.parquet", sleep: float = 0.1) -> Tuple[int, int]:
    """
    ...
    for npi in missing_npis:
        ...
        if rec:
            ...
            upsert_dim_npi(dim_df, dim_path)
            upsert_dim_npi_address(addr_df, dim_path.parent / "dim_npi_address" / "dim_npi_address.parquet")
            updated += 1
        else:
...
def sync_npis_from_xref(silver_dir: str | Path, dim_npi_filename: str = "dim_npi.parquet", sleep: float = 0.1) -> Tuple[int, int, int]:
    """
    ...
    """
    # Step 1: Get all NPIs from xref_group_npi
    xref_path = Path(silver_dir) / "xref_group_npi" / "xref_group_npi.parquet"
    ...
    print(f"Found {len(all_npis)} unique NPIs in xref_group_npi")
    
    # Step 2: Add placeholders for missing NPIs
    placeholders_added = add_npi_placeholders(all_npis, silver_dir, dim_npi_filename)
    print(f"Added {placeholders_added} placeholder records")
    
    # Step 3: Backfill unfetched NPIs
    found, updated = backfill_missing_npis(silver_dir, dim_npi_filename, sleep)
...
def main():
    ...
    ap.add_argument("--silver-dir", required=True, help="Path to your core/data/silver directory")
    ...
    ap.add_argument("--sync", action="store_true", help="Complete sync: add placeholders from xref_group_npi, then backfill")
    ap.add_argument("--placeholders-only", action="store_true", help="Only add placeholders for NPIs from xref_group_npi")
    args = ap.parse_args()

    silver = Path(args.silver_dir)
    if not silver.exists():
        print(f"Creating silver directory {silver} ...")
        silver.mkdir(parents=True, exist_ok=True)

    # Handle sync operations
    if args.sync:
        placeholders, found, updated = sync_npis_from_xref(silver, args.dim_npi_filename, args.sleep)
        print(f"Sync complete. Placeholders added: {placeholders}, Found: {found}, Updated: {updated}")
        return 0
    
    if args.placeholders_only:
        xref_path = silver / "xref_group_npi" / "xref_group_npi.parquet"
        if not xref_path.exists():
            print(f"xref_group_npi not found at {xref_path}")
            return 1
        ...
        xref_df = pl.read_parquet(str(xref_path))
        all_npis = xref_df.select("npi").unique().to_series().to_list()
        placeholders_added = add_npi_placeholders(all_npis, silver, args.dim_npi_filename)
        print(f"Added {placeholders_added} placeholder records")
        return 0
    
    if args.backfill:
        found, updated = backfill_missing_npis(silver, args.dim_npi_filename, args.sleep)
    ...
    found, wrote = add_many_npis(npis, silver, sleep=args.sleep, dim_npi_filename=args.dim_npi_filename)
